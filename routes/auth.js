...
router.post('/signup', async (req, res) => {
  try {
    ...
    const [result] = await pool.execute(
      'INSERT INTO Cliente (nome, email, senha, telefone, is_admin) VALUES (?, ?, ?, ?, ?)',
      [name.trim(), email, plainPassword, tel, false]
    );
...
router.post('/login', async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute('SELECT id_cliente, nome, email, senha, is_admin FROM Cliente WHERE email = ?', [email]);
    if (rows.length === 0) {
